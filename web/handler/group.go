...
func (h *group) Get(response trama.Response, r *http.Request) {
	if len(r.FormValue("id")) == 0 {
		response.ExecuteTemplate("group.html",
			data.NewGroup(h.Session().User.Name, data.MenuGroups))
		return
	}
	...
	group, err := groupDAO.FindById(id)
	...
	data := data.NewGroup(h.Session().User.Name, data.MenuGroups)
	data.Group = group
	response.ExecuteTemplate("group.html", data)
}
